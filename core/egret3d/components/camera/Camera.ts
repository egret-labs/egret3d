namespace egret3d {
    ...
    export class Camera extends paper.BaseComponent implements ITransformObserver {
        ...
        public get postprocessingRenderTarget(): RenderTexture {
            if (!this._readRenderTarget) {
                this._readRenderTarget = RenderTexture.create({ width: stage.viewport.w, height: stage.viewport.h }).setLiner(true).setRepeat(false).retain();
            }

            if (!this._writeRenderTarget) {
                this._writeRenderTarget = RenderTexture.create({ width: stage.viewport.w, height: stage.viewport.h }).setLiner(true).setRepeat(false).retain();
            }
